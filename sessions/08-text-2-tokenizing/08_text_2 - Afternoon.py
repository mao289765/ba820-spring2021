...
charvec = CountVectorizer(analyzer='char', ngram_range=(3,3))
...
custom_tokens = cv.fit_transform(a)
cv.get_feature_names()
...
social_tokens = []
for doc in corpus:
    social_tokens.append(social.tokenize(doc))
...
bow_df = pd.DataFrame(bow.toarray(), columns=combined.get_feature_names())
bow_df
...
idf_df = pd.DataFrame(idf.toarray(), columns=tfidf.get_feature_names())


# we could even heatmap this to help understand the intuition here

plt.figure(figsize=(4,6))
sns.heatmap(idf_df.T, xticklabels=True, yticklabels=True, cmap='Reds')
plt.show()
...
k2 = KMeans(2)
labs = k2.fit_predict(t2)


df = pd.DataFrame(t2, columns=['x1', 'x2'])
df['labs'] = labs

# sns.heatmap(x="x1", y="x2", hue="labs", data=df
sns.scatterplot(x="x1", y="x2", hue="labs", data=df)
plt.show()

...
#                 - reinforce text prep
#                 - Cluster documents is a natural next step
...
tokens = ngram2.fit_transform(corpus)
# tokens.toarray()
ngram2.vocabulary_

df = pd.DataFrame(tokens.toarray(), columns=ngram2.get_feature_names())
df
...
article_ngrams = cv.fit_transform([article.text])

article_ngrams.shape
...
## OF COURSE, you could always download your own.  not the format of below, we just pass in a list in the end
...
# last, we can always add to the stoplist if we wanted to now that its a list above
